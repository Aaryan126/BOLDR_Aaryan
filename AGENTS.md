...
- Current implementation reaches Phase 11: local ingestion, deterministic classification, explainable retrieval evidence, structured AI provider contracts, evidence-gated reply drafting, stable workflow APIs, an interactive workbench UI, a reviewable knowledge-gap/FAQ loop, theme radar, monthly marketing intelligence, and an evaluation scorecard.
- Retrieval APIs live under `/api/retrieval/*`, AI config/status APIs live under `/api/ai/*`, draft review APIs live under `/api/drafts/*`, workflow APIs live under `/api/tickets`, `/api/gaps`, and `/api/workflow/overview`, insight APIs live under `/api/themes/radar` and `/api/marketing-briefs/*`, and quality metrics live under `/api/evaluation/scorecard`.
