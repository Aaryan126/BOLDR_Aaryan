...
- Current implementation reaches Phase 8: local ingestion, deterministic classification, explainable retrieval evidence, structured AI provider contracts, evidence-gated reply drafting, stable workflow APIs, and an interactive workbench UI.
- Retrieval APIs live under `/api/retrieval/*`, AI config/status APIs live under `/api/ai/*`, draft review APIs live under `/api/drafts/*`, and workflow APIs live under `/api/tickets`, `/api/gaps`, and `/api/workflow/overview`.
